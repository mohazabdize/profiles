...
export function DocumentUploader({
  ...
  const simulateUpload = useCallback(async (file: Omit<DocumentFile, 'id' | 'status'>) => {
    setIsUploading(true);
    setUploadProgress(0);
  ...
  }, [allowedTypes, documentType, maxSizeMB, simulateUpload]);
  ...
  }, [onUploadError, handleFileSelection]); // Fixed: Added handleFileSelection dependency

  const handleRetry = useCallback(() => {
    if (document) {
      simulateUpload(document);
    }
  }, [document, simulateUpload]);
